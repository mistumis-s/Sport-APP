...
function WellnessObservaciones({ w }) {
  ...
  return (
    <div className="space-y-1.5 border-t border-slate-100 pt-2 mt-1">
      ...
      {enfermedad && <p className="text-xs"><span className="text-slate-400">Enfermedad: </span><span className="text-red-600 font-bold">{enfermedad}</span></p>}
      {sensacion && <p className="text-xs"><span className="text-slate-400">Próximo entreno: </span><span className={`font-bold ${sensacion.color}`}>{sensacion.label}</span></p>}
...
export default function PlayerEvolution() {
  ...
  if (loading) return <div className="text-center text-slate-400 py-12 font-medium">Cargando tu evolución...</div>;
  if (!data) return <div className="text-center text-red-500 py-12 font-medium">No se pudieron cargar los datos</div>;
  ...
  const combinedDataFiltered = combinedData.filter(d => d.date >= averageSince && d.date <= averageEndDate);
  ...
  return (
    <div className="pb-20 sm:pb-0 space-y-4">
      <Link to="/player" className="inline-flex items-center text-slate-400 hover:text-slate-700 text-sm font-semibold transition-colors">
        ← Volver
      </Link>

      <div className="bg-gradient-to-r from-red-500 to-rose-500 rounded-2xl p-5 text-white shadow-lg shadow-red-200">
        <p className="text-red-100 text-xs font-semibold uppercase tracking-wider">Mi evolución</p>
        <h1 className="text-2xl font-extrabold mt-1">{player.name}</h1>
        <p className="text-red-100 text-sm mt-0.5">Últimos 60 días · DH Élite</p>
      </div>
      ...
      <div className="card">
        <div className="flex items-center gap-3 flex-wrap">
          ...
          />
          <button type="button" onClick={() => setSelectedDate(maxDate)} className="text-xs px-3 py-1.5 rounded-lg bg-red-50 text-red-500 hover:bg-red-100 font-semibold transition-colors">
            Hoy
          </button>
          {selectedDate && (
            <button onClick={() => setSelectedDate('')} className="text-xs text-slate-400 hover:text-slate-700 font-semibold transition-colors">✕ Limpiar</button>
          )}
        </div>
        ...
        <div className="space-y-4">
          ...
          <div className="card">
            <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
              <p className="text-sm font-bold text-slate-800">Mis medias</p>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                <tr>
                  ...
                  <td className="text-center py-2 font-extrabold" style={{ color: wsScoreColor(wsAvg) }}>{formatAverage(wsAvg)}</td>
          ...
          {combinedDataFiltered.length > 0 && (
            <div className="card">
              <p className="text-sm font-bold text-slate-800 mb-3">WS vs RPE · últimas {averageWeeks} semana{averageWeeks > 1 ? 's' : ''}</p>
